...
func (d taskDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	if isCursor {
		cursor := "> "
		if isBlocked {
			rendered = selectedStyle.Render(cursor) + blockedStyle.Render(title) + indicatorStyle.Render(indicators)
		} else {
			rendered = selectedStyle.Render(cursor+title) + indicatorStyle.Render(indicators)
		}
	} else {
		cursor := "  "
		if isBlocked {
			rendered = cursor + blockedStyle.Render(title) + indicatorStyle.Render(indicators)
		} else {
			rendered = cursor + normalStyle.Render(title) + indicatorStyle.Render(indicators)
		}
	}
